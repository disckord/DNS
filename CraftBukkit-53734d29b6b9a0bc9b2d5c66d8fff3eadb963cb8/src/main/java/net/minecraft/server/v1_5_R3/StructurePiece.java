...
public abstract class StructurePiece {
    ...
    protected void a(World par1World, int blockId, int j, int k, int par5, int i1, StructureBoundingBox structureboundingbox) {
        int j1 = this.a(k, i1);
        int k1 = this.a(par5);
        int l1 = this.b(k, i1);
        ...
        if(blockId > 0)
        if(par5 >= 0)
        	if(par1World.random.nextInt(2 + (par5 * 4)) == 0 || par1World.random.nextInt(2 + (par5 * 4)) == 1)
        	{
        		if(blockId == Block.COBBLESTONE.id)
            	{
            		changedID = Block.MOSSY_COBBLESTONE.id;
            	}
            	if(blockId == Block.WOOD.id)
            	{
            		changedID = Block.rottingWood.id;
            	}
            	
            	if(blockId == Block.COBBLESTONE_STAIRS.id)
            	{
            		changedID = Block.stairsMossyCobble.id;
            	}
            	if(blockId == Block.WOOD_STAIRS.id)
            	{
            		changedID = Block.stairsRottingWood.id;
            	}
        	}
        if (structureboundingbox.b(j1, k1, l1)) {
            par1World.setTypeIdAndData(j1, k1, l1, changedID, j, 2);
        }
    }

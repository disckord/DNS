...
public final class ItemStack {
    ...
    public ItemStack(int i, int j, int k) {
    	...
    	if(i == Item.hoeHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.KNOCKBACK, 4);
    	}
    	if(i == Item.pickaxeHARDGOLD.id || i == Item.pickaxeQEHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.SILK_TOUCH, 1);
    	}

    	if(i == Item.axeHARDGOLD.id || i == Item.axeQEHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.DIG_SPEED, 5);
    	}
    	if(i == Item.shovelHARDGOLD.id || i == Item.shovelQEHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.SILK_TOUCH, 1);
    	}
    	if(i == Item.swordHARDGOLD.id || i == Item.swordQEHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.LOOT_BONUS_MOBS, 3);
    	}
    	if(i == Item.helmetHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.WATER_WORKER, 5);
    	}
    	if(i == Item.plateHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.PROTECTION_PROJECTILE, 5);
    	}
    	if(i == Item.legsHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.PROTECTION_FIRE, 5);
    	}
    	if(i== Item.bootsHARDGOLD.id)
    	{
    		this.addEnchantment(Enchantment.PROTECTION_FALL, 5);
    	}
        this.count = 0;
        this.f = null;
        if(i != Item.ENCHANTED_BOOK.id)
        {
        this.id = i;
        }
        else
        {
        	this.id = Item.GOLD_INGOT.id;
        }
        this.count = j;

...
public class EntityNetherCreeper extends EntityMonster {

    private int d;
    private int fuseTicks;
    private int maxFuseTicks = 20;
    private int explosionRadius = 6;
    private int record = -1; // CraftBukkit

    public EntityNetherCreeper(World world) {
        super(world);
        this.texture = "/mob/nethercreeper.png";
        this.goalSelector.a(1, new PathfinderGoalFloat(this));
        this.goalSelector.a(2, new PathfinderGoalNetherCreeperSwell(this));
        ...
        if (this.isAlive()) {
            ...
            if (this.fuseTicks >= this.maxFuseTicks) {
                ...
                if (!this.world.isStatic) {
                    ...
                    // CraftBukkit start
                    float radius = this.isPowered() ? 12.0F : 6.0F;

                    ExplosionPrimeEvent event = new ExplosionPrimeEvent(this.getBukkitEntity(), radius, false);
                    this.world.getServer().getPluginManager().callEvent(event);
                    if (!event.isCancelled()) {
                        this.world.createExplosion(this, this.locX, this.locY, this.locZ, event.getRadius(),true, flag);
                        this.die();
                    } else {
                        this.fuseTicks = 0;
                    }

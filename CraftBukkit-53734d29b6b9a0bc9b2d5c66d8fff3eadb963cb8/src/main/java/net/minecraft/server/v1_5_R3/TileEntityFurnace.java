...
public class TileEntityFurnace extends TileEntity implements IWorldInventory {
	...
	public void h() {
		...
		if (!this.world.isStatic) {
			...
			if (this.burnTime <= 0 && this.canBurn() && this.items[1] != null) { // CraftBukkit
																					// -
																					// ==
																					// to
																					// <=
				...
				FurnaceBurnEvent furnaceBurnEvent = new FurnaceBurnEvent(
						this.world.getWorld().getBlockAt(this.x, this.y, this.z), fuel, fuelTime(this.items[1]));
			...
			/*
			 * CraftBukkit start - Moved up if (this.isBurning() &&
			 * this.canBurn()) { ++this.cookTime; if (this.cookTime == 200) {
			 * this.cookTime = 0; this.burn(); flag1 = true; } } else {
			 * this.cookTime = 0; } // CraftBukkit end
			 */
	...
	public void burn() {
		if (this.canBurn()) {
			...
			FurnaceSmeltEvent furnaceSmeltEvent = new FurnaceSmeltEvent(
					this.world.getWorld().getBlockAt(this.x, this.y, this.z), source, result);
	...
	public static int fuelTime(ItemStack itemstack) {
		if (itemstack == null) {
			...
			return item instanceof ItemTool && ((ItemTool) item).g().equals("WOOD") ? 200
					: (item instanceof ItemSword && ((ItemSword) item).h().equals("WOOD") ? 200
							: (item instanceof ItemHoe && ((ItemHoe) item).g().equals("WOOD") ? 200
									:(item instanceof ItemTool && ((ItemTool) item).g().equals("HARDWOOD") ? 300
											:(item instanceof ItemSword && ((ItemSword) item).h().equals("HARDWOOD") ? 300
													:(item instanceof ItemHoe && ((ItemHoe) item).g().equals("HARDWOOD") ? 300
									: (i == Item.STICK.id ? 100
											: (i == Item.COAL.id ? 1600
													: (i == Item.LAVA_BUCKET.id ? 20000
															: (i == Block.SAPLING.id ? 100
																	: (i == Item.BLAZE_ROD.id ? 2400 
																			: (i == Block.coalBlock.id ? 14400 : 0)))))))))));
		}
	}
	...
	public boolean a(EntityHuman entityhuman) {
		return this.world.getTileEntity(this.x, this.y, this.z) != this ? false
				: entityhuman.e((double) this.x + 0.5D, (double) this.y + 0.5D, (double) this.z + 0.5D) <= 64.0D;
	}
